...
private ObservableList<tableUser> originalUserList; // Declare a variable to store the original user list
        ...
        if (selectedUser != null) {
            ...
            if (result.isPresent() && result.get() == ButtonType.OK) {
                ...
                if (success) {
                    ...
                    showErrorAlert( "Error", "Failed to remove the user.");
                }
            }
        } else {
    ...
    void handle_Register_Button(ActionEvent event) {
        ...
        if (buttonText.equals("Save")) {
            ...
            try (Connection connection = DatabaseConnection.getConnection();
                ...
                boolean disable;
                int Role = 0;
                if(Disability_Button.isSelected()){disable = true;} else {disable = false;}
                if (Choicebox_Role.getValue() == "Admin") { Role= 1;}else if(Choicebox_Role.getValue() == "User") {Role=2;}
    ...
    private boolean isUserExistsWithPassword(String password) {
        // Implement your logic to check if a user with the same password already exists in the database
        // Perform the necessary database query and return true if a user exists with the same password, false otherwise
        // For example:
        try {
            DatabaseConnection connectionNow = new DatabaseConnection();
            Connection connectionDB = connectionNow.getConnection();
